...
class ProfileController extends Controller
{
    ...
    public function uploadProfilePicture(Request $request)
    {
        try {
            ...
            if ($etudiant) {
                $anneeUni = $etudiant->annee_uni;
                $filename = $etudiant->apogee . '.' . $extension;
                $directory = 'public/avatars/' . $anneeUni;
            } else {
                $filename = str_replace(' ', '', $user->name) . '.' . $extension;
                $directory = 'public/avatars/personnel/' . date('Y');
            }
    ...
    public function usercard($id_etu)
    {
        ...
        return view('usercard', compact(
            ...
            'etudiantEtape',
            'active_tab',
            'is_uploaded_initiation',
            'is_uploaded_technique',
            'is_uploaded_pfe',
            'is_uploaded_professionelle'
        ));
    }
}
